        ...
        <Link to={`/proyectos/${slug}`} className="w-full h-full block">
          ...
          {imageUrl && imageUrl.trim() !== "" ? (
            ...
            wireframeType === 'mobile' ? (
              ...
              <div className="relative w-full h-full bg-linear-to-br from-brand-surface-subtle to-brand-bg flex items-end justify-center overflow-hidden">
                 <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-48 h-48 bg-brand-accent/15 blur-3xl rounded-full pointer-events-none"></div>
                 <div className="relative h-[120%] sm:h-[130%] aspect-[9/19.5] bg-brand-bg rounded-t-[1.2rem] sm:rounded-t-[1.5rem] border-[3px] sm:border-[4px] border-b-0 border-brand-border shadow-brand-accent/30 overflow-hidden transition-transform duration-500 group-hover:-translate-y-3">
                    <div className="absolute top-[20%] -right-[3px] sm:-right-[4px] w-[2px] h-6 sm:h-8 bg-brand-border rounded-l-md"></div>
