...
`};function i(i){e.style.opacity=`0`,e.style.transform=`translateX(-20px)`,setTimeout(()=>{e.innerHTML=r[i]||r.Proyectos;let t=e.scrollHeight;n.style.height=`${t}px`,e.style.transform=`translateX(20px)`,requestAnimationFrame(()=>{e.style.opacity=`1`,e.style.transform=`translateX(0)`})},200),t.forEach(e=>{e.className=`flex-1 pb-3 text-[12px] md:text[15px] font-semibold transition-all whitespace-nowrap ${e.textContent.trim()===i?`text-white border-b border-white`:`text-zinc-500 hover:text-zinc-300`}`})}i(`Proyectos`),t.forEach(e=>{e.addEventListener(`click`,()=>i(e.textContent.trim()))});